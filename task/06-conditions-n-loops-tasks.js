...
function isCreditCardNumber(ccn) {
    ...
    let sum = 0;       
    for(let i = 1; i <= len; i++) {
        let digit = Number.parseInt(ccnString[len - i])
        if(i % 2 === 0) {
            digit *= 2;
            if (digit > 9) digit -= 9;
        }
...
function isBracketsBalanced(str) {
    ...
    for(let i = 0; i < str.length; i++) {
        let bracket = str[i];
        if(openBracket.indexOf(bracket) !== -1) stack.push(bracket);
        else if(closeBracket.indexOf(bracket) !== -1) {            
            if(closeBracket.indexOf(bracket) !== openBracket.indexOf(stack.pop())) return false;                                   
        }       
    }
 ...
 *
 */
function getMatrixProduct(m1, m2) {  // only if m1[0].length === m2.length
    let result = [];    
